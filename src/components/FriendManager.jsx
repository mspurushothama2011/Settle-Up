...
import { UserPlus, User, Phone, RefreshCw, AlertCircle, Sparkles, ArrowUpRight, ArrowDownLeft, Check } from 'lucide-react';
...
export default function FriendManager({ currentUser, refreshTrigger }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-slate-900 dark:text-slate-100 max-w-5xl mx-auto">
      {/* Add Friend Form */}
      <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm h-fit transition-colors duration-150">
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 text-red-650 dark:text-red-400 rounded-lg text-xs flex items-start gap-2">
      ...
      <div className="md:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-150">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-slate-550 dark:text-slate-450" />
            Group Directory & Standings ({friends.length})
          </h2>
          <button
            onClick={fetchData}
            disabled={fetching}
            className="text-slate-450 hover:text-slate-655 dark:text-slate-500 dark:hover:text-slate-350 transition-colors"
          >
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500 font-mono truncate mt-0.5">{friend.phone}</p>
                    </div>
                  </div>
                    ...
                    <div className="text-right flex-shrink-0 pl-3">
                      {balance > 0.01 ? (
                        ...
                        <div className="flex flex-col items-end opacity-70">
                          <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 font-mono flex items-center gap-0.5">
                            <Check className="w-3.5 h-3.5 text-slate-400" />
                            ₹0.00
                          </p>
                          <p className="text-[9px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-0.5">
                            Settled
                          </p>
                        </div>
                      )}
